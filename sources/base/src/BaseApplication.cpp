...
BaseApplication::BaseApplication(std::string_view appName, std::optional<std::string_view> orgName)
: mStopping{false}
, mAppName{appName}
, mOrgName{orgName}
{
...
int BaseApplication::run(int argc, char ** argv)
{
    ...
    {
#if not defined(SFML_SYSTEM_ANDROID)
    ...
    try
    {
        if (!error)
        {
            ...
            if (!error)
            {
                logger = mContext.service<Logging>()->logger();
            }
        }
    }
...
std::optional<fs::path> BaseApplication::findConfig()
{
...
int BaseApplication::configure(int argc, char ** argv)
{
    ...
    else
    {
        ...
        {
            auto logNode = rootNode.child("logging");
            if (logNode)
            {
                ...
                if (!levelAttr.empty())
                {
                    ...
                    {
                        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Logging config is malformed: unexpected level value: '%s'", levelStr.data());
...
// 				{
// 					SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Logging config is malformed: unexpected multithreaded value: '%s'", mtStr.data());
// 				}
// 			}
...
// 						{
// 							SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Logging/sinks/stdout config is malformed: unexpected color value: '%s'", colorStr.data());
// 						}
// 					}
...
// 						if (!stru::parseBool(colorStr, colored))
// 						{
// 							SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Logging/sinks/stderr config is malformed: unexpected color value: '%s'", colorStr.data());
// 						}
// 					}
...
// 						if (name.empty())
// 						{
// 							SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Logging/sinks/file config is malformed: name is empty");
// 						}
// 						else
// 						{
// 							fileName = name;
...
// 						{
// 							SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Logging/sinks/file config is malformed: unexpected color value: '%s'", truncateStr.data());
// 						}
// 					}
...
// 		}
...
#if not defined(SFML_SYSTEM_WINDOWS)
        multithreaded
                ? sinks.push_back(std::make_shared<spdlog::sinks::stdout_sink_mt>())
                : sinks.push_back(std::make_shared<spdlog::sinks::stdout_sink_st>());
    #endif
    ...
    try
    {
        auto logging = mContext.addService<Logging>();
        logging->configure(sinks, pattern, level);
    }
    catch (std::exception const & ex)
    {
    ...
    Logger logger = mContext.service<Logging>()->logger();
...
void BaseApplication::shutDown()
{
    ...
    try
    {
        //TODO: base::BaseApplication-level shutdown
    }
    catch (std::system_error const & ex)
    {
...
//void BaseApplication::findBasePath()
//{
//#if defined(SFML_SYSTEM_LINUX)
...
//void BaseApplication::findPrefPath()
//{
//#if defined(SFML_SYSTEM_LINUX)
//    fs::path path;
